...
int
getStates(struct ipv4_5tuple *ip_5tuple, struct nf_states ** state, void* callback_arg)
{
    union ipv4_5tuple_host newkey;
    convert_ipv4_5tuple(ip_5tuple, &newkey);
    ...
    if (ret >= 0) {
        ...
        if (ret >= 0) {
            ...
            pullState(1, 0, callback_arg, ip_5tuple, index, state);
            ret = 0x36;
        }
        else {
            #ifdef __DEBUG_LV1
...
int
pullStateCallback(struct nf_states* state, void* callback_arg)
{
            ...
            for (i = 0; i < nb_rx; i ++) {
                ...
                if (ip_5tuples.proto == 17) {
                    ...
                    if (tcp_hdrs->tcp_flags == 0x12 || tcp_hdrs->tcp_flags == 0x2) {
                        ...
                        int ret =  getStates(&ip_5tuples, &state, (void*)bufs[i]);
                        //printf("%x\n", state);
                        ...
                        if (ret == 0) {
                            ...
                            #ifdef __DEBUG_LV1
                            printf("nf: pull state from remote\n");
                            #endif
                        }
                        else {
                            rte_pktmbuf_free(bufs[i]);
